...
// Sidebar badge: number of new inquiries
$new_inq_count = 0;
// if ($result = @mysqli_query($conn, "SELECT COUNT(*) AS c FROM inquiries WHERE status = 'new'")) {
...
<body class="min-h-screen flex">
  ...
  <aside id="sidebar" class="w-60 bg-ink-2 border-r border-ink-3 flex flex-col h-screen sticky top-0 flex-shrink-0">
    ...
    <div class="px-6 py-6 border-b border-ink-3">
      <a href="<?= APP_URL ?>" class="display text-2xl"><?= htmlspecialchars(APP_NAME) ?><span class="text-gold">.</span></a>
      <div class="eyebrow text-cream-3 mt-1">Producer · Admin</div>
    </div>

    <nav class="py-4 flex-1 overflow-y-auto">
      <div class="eyebrow text-cream-3 px-6 mb-2 mt-2">Manage</div>
      ...
      <a href="<?= APP_URL ?>admin/inquiries/" class="hidden <?= admin_link_class('inquiries') ?><?= $current_url == APP_URL . 'admin/inquiries/' ? ' active' : '' ?>">
        <span class="flex items-center gap-3">
          <svg class="icon" viewBox="0 0 16 16" fill="none">
      ...
      <a href="<?= APP_URL ?>admin/tracks/" class="<?= admin_link_class('tracks') ?><?= $current_url == APP_URL . 'admin/tracks/' || $current_url == APP_URL . 'admin/tracks/add/' ? ' active' : '' ?>">
        <span class="flex items-center gap-3">
          <svg class="icon" viewBox="0 0 16 16" fill="none">
      ...
      <a href="<?= APP_URL ?>admin/services/" class="hidden <?= admin_link_class('services') ?><?= $current_url == APP_URL . 'admin/services/' ? ' active' : '' ?>">
        <span class="flex items-center gap-3">
          <svg class="icon" viewBox="0 0 16 16" fill="none">
      ...
      <a href="<?= APP_URL ?>admin/availability/" class="hidden <?= admin_link_class('availability') ?><?= $current_url == APP_URL . 'admin/availability/' ? ' active' : '' ?>">
        <span class="flex items-center gap-3">
          <svg class="icon" viewBox="0 0 16 16" fill="none">
      ...
      <a href="<?= APP_URL ?>admin/press/" class="hidden <?= admin_link_class('press') ?><?= $current_url == APP_URL . 'admin/press/' ? ' active' : '' ?>">
        <span class="flex items-center gap-3">
          <svg class="icon" viewBox="0 0 16 16" fill="none">
            <path d="M2 3h12v10H2zM2 6h12M5 9h6M5 11h4" stroke="currentColor" stroke-width="1.5" />
          </svg>
          Press kit
        </span>
      </a>
